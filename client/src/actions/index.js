...
function getPreferences(session) {
  // console.log('getPreferences', session);
  return function (dispatch) {
    ...
    return function (dispatch) {
      fetch(`/authenticate/${authToken}`)
        ...
        .then((response) => {
          ...
          if (!response.ok) {
            dispatch(loadMessage({ ok: false, status: 'User', statusText: 'User could not be found' }, 'ERROR'));
            localStorage.removeItem('token');
          } else {
        ...
        .catch((err) => {
          ...
          dispatch(loadMessage({ ok: false, status: 401, statusText: 'Caught unknown authorization error' }, 'ERROR'));
          localStorage.removeItem('token');
