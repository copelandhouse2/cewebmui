...
export * from './ceFieldGroupRevision';
    ...
    <Grid container justify='center'>
      ...
      <Tooltip title={`Press button to change scope items.`} placement={'right-start'}>
        <Fab
          ...
          onClick={() => {
            props.toggleScopeDialog(props.setState, true);
          }}>
  ...
  class extends Component {
    ...
    componentDidMount = () => {
      const columns = this.props.fieldGroup.children.map((c) => {
        ...
        const formatter =
          c.column_formatter === 'jobColFormatter'
            ? this.jobNumColFormatter
            : c.column_formatter === 'centerColFormatter'
            ? this.centerColFormatter
            : this.colFormatter;
    ...
    headerColFormatter = (value) => {
      return <div className={this.props.classes.headerColumn}>{value}</div>;
    };

    jobNumColFormatter = ({ value }) => {
      return <Typography className={this.props.classes.jobNumColumn}>{value ? value : 'Current Entry'}</Typography>;
    };

    centerColFormatter = ({ value }) => {
      return <Typography className={this.props.classes.centerColumn}>{value}</Typography>;
    };

    colFormatter = ({ value }) => {
      return <Typography className={this.props.classes.column}>{value}</Typography>;
    };
            ...
            <TableHead>
              <TableRow>
                {subFG.children.map((c, i) => {
                  return <TableCell key={i}>{c.label}</TableCell>;
                })}
              </TableRow>
            </TableHead>
            <TableBody>
              {pRow[subGroupKey].map((obj, si) => (
                <TableRow key={si}>
                  {subFG.children.map((c, i) => {
                    return <TableCell key={i}>{obj[c.name]}</TableCell>;
                  })}
                </TableRow>
              ))}
            </TableBody>
...
export const MaterialTabularFG = withStyles(styles, { withTheme: true })(
  class extends Component {
    ...
    onDragEnd = (result) => {
      ...
      const items = reorder(this.state.columns, result.source.index, result.destination.index);
    ...
    handleEdit = (row) => {
      ...
      const theRow = Object.assign({}, { ...editRow, change: editRow.id ? 'edit' : 'add', last_updated_by: this.props.parentState.last_updated_by });
    ...
    render() {
      ...
      let hdr0 = classes.hCol0,
        hdr1 = classes.hCol1,
        hdr2 = classes.hCol2,
        hdr3 = classes.hCol3;
      let clm0 = classes.col0,
        clm1 = classes.col1,
        clm2 = classes.col2,
        clm3 = classes.col3;
              ...
              <Table>
                <TableHead>
                  <TableRow component={DroppableComponent(this.onDragEnd, classes)}>
                    {subGroupKey && <TableCell padding='checkbox' classes={{ root: `${classes.tableCellHeaderProps} ${hdr0}` }} />}
                    {parentState.selectedIndexes && <TableCell padding='checkbox' classes={{ root: `${classes.tableCellHeaderProps} ${hdr1}` }} />}
                  ...
                  {data.map((r, ri) => {
                    ...
                    return (
                      <Fragment key={ri}>
                        <TableRow key={ri}>
                          {subGroupKey && (
                            <TableCell padding='checkbox' className={`${classes.tableActionProps} ${clm0}`}>
                              <IconButton aria-label='Expand' onClick={() => this.handleExpand(ri)} className={classes.actionButtons}>
                          ...
                          <TableCell padding='checkbox' className={`${classes.tableActionProps} ${clm2}`}>
                            {!this.state.editedRows[ri] && ( //parentState.editMode[ri]
                              <Tooltip title={editHelp ? editHelp : 'Edit the project here'} aria-label='Edit the project here'>
                                <IconButton aria-label='Expand' onClick={() => this.handleEdit(ri)} className={classes.actionButtons}>
                                  <Edit />
                                </IconButton>
                              </Tooltip>
                            )}
                            {this.state.editedRows[ri] && ( //parentState.editMode[ri]
                              <Tooltip
                                title={
                                  saveHelp
                                    ? saveHelp
                                    : 'Just Save the project.  Changes not sent to Trello or Box yet.  Project status will be PENDING'
                                }
                                aria-label='Just Save the project.  Changes not sent to Trello or Box yet.  Project status will be PENDING'>
                                <IconButton aria-label='Expand' onClick={() => this.handleSave(ri)} className={classes.actionButtons}>
                                  <Save />
                                </IconButton>
                              </Tooltip>
                            )}
                          ...
                          <TableCell padding='checkbox' className={`${classes.tableActionProps} ${clm3}`}>
                            {!this.state.editedRows[ri] && ( //parentState.editMode[ri]
                              <Tooltip title={deleteHelp ? deleteHelp : 'Delete the project'} aria-label='Delete the project'>
                                <IconButton aria-label='Expand' onClick={() => this.handleDelete(ri)} className={classes.actionButtons}>
                                  <Delete />
                                </IconButton>
                              </Tooltip>
                            )}
                            {this.state.editedRows[ri] &&
                              !(this.props.allowAdd && ri === 0) && ( //parentState.editMode[ri]
                                <Tooltip title='Cancel changes' aria-label='Cancel changes'>
                                  <IconButton aria-label='Expand' onClick={() => this.handleCancel(ri)} className={classes.actionButtons}>
                                    <Cancel />
                                  </IconButton>
                                </Tooltip>
                              )}
                        ...
                        {subGroupKey && <ChildDetailTable pIdx={ri} pRow={r} subGroupKey={subGroupKey} subFG={subFG} state={this.state} />}
                      </Fragment>
                    );
                  })}
                </TableBody>
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
      ...
      if (prevState.scopeToParent !== parentState.scope) {
        ...
        if (parentState.scope !== null) {
          ...
          const sFields = subFG.children.map((s) => {
            return { ...s, scope_field: true };
          });
                ...
                <TableHead>
                  <TableRow component={DroppableComponent(this.onDragEnd, classes)}>
                    ...
                    <TableCell
                      padding='checkbox'
                      classes={{
                        root: editActive ? `${classes.tableCellHeaderProps} ${classes.hCol2xl}` : `${classes.tableCellHeaderProps} ${classes.hCol2}`,
                      }}
                    />
                    <TableCell
                      padding='checkbox'
                      classes={{
                        root: editActive ? `${classes.tableCellHeaderProps} ${classes.hCol3xl}` : `${classes.tableCellHeaderProps} ${classes.hCol3}`,
                      }}
                    />
                    {this.state.columns.map((c, i) => (
                      <TableCell
                        ...
                        classes={{
                          root:
                            i === 0
                              ? editActive
                                ? `${classes.lockCellProps} ${classes.tableCellHeaderProps} ${classes.hCol4xl}`
                                : `${classes.lockCellProps} ${classes.tableCellHeaderProps} ${classes.hCol4}`
                              : `${classes.tableCellProps} ${classes.tableCellHeaderProps}`,
                        }}>
                        {c.label}
                      </TableCell>
                    ))}
                      ...
                      <Fragment key={ri}>
                        <TableRow key={ri}>
                          ...
                          <TableCell padding='checkbox' className={`${classes.tableActionProps} ${classes.col1}`}>
                            <Tooltip
                              title='Works in conjunction with EDIT button.  Edit the project in full screen mode.'
                              aria-label='Edit the project in full screen mode'>
                              <Checkbox
                                onChange={() => this.onRowSelected(ri)}
                                checked={parentState.selectedIndexes[0] === ri}
                                className={classes.actionButtons}
                          ...
                          <TableCell padding='checkbox' className={`${classes.tableActionProps} ${classes.col2}`}>
                            ...
                            {this.state.editedRows[ri] && ( //parentState.editMode[ri]
                              <Grid item className={classes.spreadActions}>
                                <Tooltip
                                  title='Just Save the project.  Changes not sent to Trello or Box yet.  Project status will be PENDING'
                                  aria-label='Just Save the project.  Changes not sent to Trello or Box yet.  Project status will be PENDING'>
                                  <IconButton aria-label='Expand' onClick={() => this.handleSave(ri)} className={classes.actionButtons}>
                                    <Save />
                                  </IconButton>
                                ...
                                <Tooltip
                                  title='Save the project and make updates to downstream systems like Trello and Box.  Project status will be ACTIVE'
                                  aria-label='Save the project and make updates to downstream systems like Trello and Box.  Project status will be ACTIVE'>
                                  <IconButton aria-label='Expand' onClick={() => this.handleSave(ri, true)} className={classes.actionButtons}>
                          ...
                          <TableCell
                            padding='checkbox'
                            className={editActive ? `${classes.tableActionProps} ${classes.col3xl}` : `${classes.tableActionProps} ${classes.col3}`}>
                            {!this.state.editedRows[ri] && ( //parentState.editMode[ri]
                            ...
                            if (this.state.editedRows[ri] && (!c.scope_field || (c.scope_field && childID > -1))) {
                                ...
                                <TableCell
                                  ...
                                  classes={{
                                    root:
                                      ci === 0
                                        ? editActive
                                          ? `${classes.lockCellProps} ${classes.col4xl}`
                                          : `${classes.lockCellProps} ${classes.col4}`
                                        : `${classes.tableCellProps}`,
                                  }}>
                                  <ColumnContainer
                                    key={ci}
                                    field={c}
                              ...
                              return (
                                <TableCell
                                  ...
                                  {r && c.name === 'job_number'
                                    ? r[c.name]
                                    : r && c.name !== 'scope' && c.name in r
                                    ? r[c.name]
                                    : childID > -1 && c.name in r.scope[childID]
                                    ? r.scope[childID][c.name]
                                    : childID > -1 && c.name === 'revision_desc_scope' && 'revision_desc' in r.scope[childID]
                                    ? r.scope[childID]['revision_desc']
                                    : ''}
                        ...
                        <ChildDetailTable pIdx={ri} pRow={r} subGroupKey={subGroupKey} subFG={subFG} state={this.state} />
                      </Fragment>
                    );
                  })}
                </TableBody>
...
export const DefaultFG = withStyles(styles, { withTheme: true })((props) => {
  const { classes, fieldGroup, state, updateState, hide, fgTools, fieldTools, findAction, noBorder, noLabel } = props;
...
export const DialogDefaultFG = withStyles(styles, { withTheme: true })((props) => {
...
export const ListTabularFG = withStyles(styles, { withTheme: true })(
  class extends Component {
    ...
    render() {
      const { classes, title, titleStyles, fieldGroup, primaryFields, nestedFields, fgStyles, fgTools } = this.props;
                    ...
                    if (parentState.object === 'INSPECTION' && field.name === 'inspection_status') {
                      ...
                      return (
                        <ListItemText
                          ...
                          className={classes.listHeaderColumn}
                          style={{ width: field.column_width, textAlign: 'center' }}
                    ...
                    if (parentState.object === 'INSPECTION' && field.name === 'inspection_reason') {
                      ...
                      return (
                        <ListItemText
                          ...
                          primary={this.labelHeader(field, false)}
                        />
                      );
                    }
                    return (
                    ...
                    <ListItem divider={true}>
                      {listFields.map((field, field_id) => {
                        ...
                        if (parentState.object === 'INSPECTION' && field.name === 'address1') {
                          ...
                          return (
                            <ListItemText
                              ...
                              primary={
                                <span>
                                  <b style={{ fontWeight: 500 }}>{item[field.name]}</b>
                                  <b style={{ fontWeight: 300 }}>
                                    , {item.subdivision}, {item.city}
                                  </b>
                                </span>
                              }
                            />
                        ...
                        if (parentState.object === 'INSPECTION' && field.name === 'inspection_status') {
                          // console.log('testing, testing!');
                          return (
                            <ListItemText key={field_id} style={{ width: field.column_width, textAlign: 'center' }} primary={item[field.name]} />
                          );
                        }
                          ...
                          {nestedFields.map((f2, id2) => {
                            return (
                              <ListItemText
                                key={id2}
                                disableTypography={true}
                                style={{ width: f2.column_width }}
                                primary={this.labelHeader(f2, false)}
                              />
                            );
                          })}
                        </ListItem>
...
export const SimpleListView = withStyles(styles, { withTheme: true })((props) => {
    ...
    <Grid container>
      ...
      <Grid item xs={12}>
        <List dense={true}>
          <Paper classes={{ root: classes.listHeader }}>
            <ListItem divider={true}>
              {deleteRow ? <ListItemText disableTypography={true} style={{ fontSize: 13, cursor: 'default', width: 40 }} primary={'Delete'} /> : null}
...
export const SimpleSortListView = withStyles(styles, { withTheme: true })(
  class extends Component {
    ...
    sort = (field) => {
      ...
      this.setState({
        sort: { field: field.name, data_type: field.display_value_data_type ? field.display_value_data_type : field.data_type, direction: direction },
      });
    };
          ...
          <Grid item xs={12}>
            <List dense={true}>
              <Paper classes={{ root: classes.listHeader }}>
                <ListItem divider={true}>
                  {deleteRow ? (
                    <ListItemText disableTypography={true} style={{ fontSize: 13, cursor: 'default', width: 40 }} primary={'Delete'} />
                  ) : null}
                  {fields.map((field, id1) => {
                    return (
                      <ListItemText
...
export const SimpleSortNestListView = withStyles(styles, { withTheme: true })(
  class extends Component {
    constructor(props) {
      ...
      this.state = {
        // data:[...this.props.data],
        sort: this.props.sort || { field: this.props.fields[0].name, data_type: this.props.fields[0].data_type, direction: 'A' },
      };
