...
const colSelectProps = theme => ({
  ...
  control: (provided, state) => ({ ...provided,  minHeight: 0, height:32, fontSize: 12, backgroundColor: 'rgba(0, 0, 0, 0.09)'
  , boxShadow: '0px 0px 0px 0px #fafafa'
  , borderWidth: 0
...
const findProjectHelp = () => {
  return (
    ...
    Also, values are case insensitive.  You can use uppercase or lowercase.
    </Typography>
    ...
    status:  - Status: PENDING or ACTIVE<br />
    </Typography>
    <Typography variant='body1' gutterBottom={true}>
    You can also search the scope of each project for specific values.
    </Typography>
    <Typography variant='subtitle2' noWrap={true}>
    rdescs: or revision_desc_scope:<br />
    pt: or plan_type:<br />
    gt: or garage_type:<br />
    gs: or garage_swing:<br />
    ft: or foundation_type:<br />
    flrt: or floor_type:<br />
    rt: or roof_type:<br />
    scope: <i>Scope keywords to use:</i><br />
...
const handleBlur = (name, props) => {
  ...
  switch (name) {
    ...
    case 'block':
    case 'lot':
      if (dupCheck && state[name] !== state.saveValue
    ...
    <Grid item xs={12} md={field.display_width} style={ {zIndex: field.z_index } }>
      <FormControl fullWidth={true} variant='outlined' >
        ...
        <CreatableSelect styles={createSelectProps(theme)} //styles={createSelectAutoFillProps}                  isClearable
          ...
          onCreateOption={(selected) => {props.createDialogValue?
            props.createDialogValue(selected, field):
            false
            }
          }
  ...
  return (
    <TextField
      ...
      InputProps={{
        ...
        endAdornment:field.name === 'job_number' && !searchMode?
          <IconButton
            aria-label='job-unlock'
            color='secondary'
            title='Unlock job number for editing'
